...
public class OutboundDocumentController {
    ...
    @PostMapping
    @Operation(summary = "Retrieve all outbound documents", description = "Post request to retrieve all outbound documents")
    public OutboundDocumentFilterResponseDto getAllOutboundDocumentsByFiltering(
            @RequestBody OutboundDocumentFilterRequestDto outboundDocumentFilterRequestDto
    ) {
        return outboundDocumentService.getAllOutboundDocumentsByFiltering(outboundDocumentFilterRequestDto);
    }
